...
var divTemplate ="" +
    ...
    "                <img class=\"itemLogo\" src=\"/static/Images/qingxi.png\">\n" +
...
{
    Emplyee_Login_Phone = $.cookie('Emplyee_Login_Phone');

    $.get_current_query(currentLoadType);
...
// 自定义函数
$.extend({
    get_current_query:function (state) {

        $.get("/api/pub/?Command=Query_Orders&ecode=" + Emplyee_Login_Phone + "&pageindex=" + currentPageIndex + "&pagesize=" + currentPageSize + "&state=" +state+"&type=9",
            function (data)
            {
                // 检查查询状态
